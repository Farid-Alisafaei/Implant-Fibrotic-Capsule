...
C
      ...
      REAL*8 StressRatio
C
      REAL*8 Ustrain, Umotchem, Umotor, Umechfeed, Uext, Uactin, Utotal, UactinF
      REAL*8 TraceStrainC, TraceRho, TraceSigma, TraceStrainT
      REAL*8 Uecm, UecmF, TraceStrainE
      REAL*8 StabThreshV, StabThreshD
C
...
C
      ...
      BetaV          = PROPS(6)    ! volumetric chemical stiffness (1/KPa)
      BetaD          = PROPS(7)    ! deviatoric chemical stiffness (1/KPa)
      AlphaV         = PROPS(8)    ! volumetric chemo-mechanical feedback parameter (1/KPa)
      AlphaD         = PROPS(9)    ! deviatoric chemo-mechanical feedback parameter (1/KPa) 
      Rho0I          = PROPS(10)    ! initial motor density in the quiescent state (KPa)
C
      ! ECM parameters
C
      EmodLamin      = PROPS(11)   ! elastic modulus (KPa)
      EnuLamin       = PROPS(12)   ! poisson's ratio
C
      ! adhesion layer parameters
C
      EmodAdhesion   = PROPS(13)   ! elastic modulus (KPa)
      EnuMdhesion    = PROPS(14)   ! poisson's ratio
C
      ! chromatin parameters
C
      EmodChromatin = PROPS(15)   ! elastic modulus (KPa)
      EnuChromatin  = PROPS(16)   ! poisson's ratio
C
C ----------------------------------------------------------------
C ----------------------------------------------------------------
C ----------------------------------------------------------------
...
C  
c      Rho0 = Rho0I + 0.75D0 * STATEV(10+4*NTENS+4)
      Rho0 = Rho0I + 1.0D0 * STATEV(10+4*NTENS+4) * Rho0I
      
C     IF (Rho0.GT.40.0D0*Rho0I) THEN
C         Rho0 = 40.0D0*Rho0I
C     END IF
C
C ----------------------------------------------------------------
...
C ----------------------------------------------------------------
C      
C fiber elastic modulus (second component) ??????How to get this?   Eq. S2.23
C
c      EmodCF = 10.0D0 * EmodCell2 * (ONE-EnuCell2) / 
c     1                              ((ONE+EnuCell2)*(ONE-TWO*EnuCell2))
      
      EmodCF = 50.0D0 * EmodCell2 * (ONE-EnuCell2) / 
...
C
      StabThreshV = 1.0D0 / (THREE * KmodC1)
      StabThreshD = 1.0D0 / (TWO * GmodC1)
C
      IF (BetaV <= AlphaV .OR. AlphaV <= StabThreshV) THEN
      CALL XIT()
      END IF
C
      IF (BetaD <= AlphaD .OR. AlphaD <= StabThreshD) THEN
      CALL XIT()
      END IF
C
C effective bulk modulus for motor density (first component)    Eq. S2.10
C
      KpBarMod = (ONE/THREE) * ( THREE * KmodC1 * AlphaV - ONE ) /
     1                         ( BetaV  - AlphaV )
C
C effective shear modulus for motor density (first component)    Eq. S2.11
C     
...
c      GmodC1 = 0
C      
C effective contractility (first component)   Eq. S2.12
C
      Rho0Bar = BetaV * Rho0 * (TEMP+DTEMP) / ( BetaV - AlphaV )     
C
C ----------------------------------------------------------------
C ----------------------------------------------------------------
C ----------------------------------------------------------------
...
C ----------------------------------------------------------------
C
C isotropic elastic stiffness matrix (first component)    ?????Is it the matix for actin?
C
      DO K1 = 1,NTENS
          DO K2 = 1,NTENS
...
C
      ...
      STATEV(61) = (STATEV(2*NTENS+1) + STATEV(2*NTENS+2) 
     1             + STATEV(2*NTENS+3))/3
C
C
...
C
     ...
     1         (KStressVal(K1).LT.KStressPrinMax) ) THEN
          ...
          END IF
      END DO
      STATEV(80)=KStressPrinMax
      STATEV(81)=KStressPrinMid
      STATEV(82)=KStressPrinMin
      STATEV(83)=KStressVal(1)
      STATEV(84)=KStressVal(2)
      STATEV(85)=KStressVal(3)
C
...
C
c      STATEV(10+4*NTENS+4) = STATEV(9+4*NTENS+4) * Rho0I  
C     STATEV(10+4*NTENS+4) = STATEV(9+4*NTENS+4) * KStressPrinMax 
      STATEV(10+4*NTENS+4) = STATEV(9+4*NTENS+4)
C
C contractile elastic stiffness matrix (first component)
C     
      STATEV(11+4*NTENS+4) = Rho0
...
C
      ...
      STATEV(62) = ( KStressPrinMax + KStressPrinMid + KStressPrinMin ) / THREE
      STATEV(63) = KStressPrinMax
      STATEV(64) = ( KRhoVal(1) + KRhoVal(2) + KRhoVal(3) ) / THREE
      STATEV(65) = MAX( KRhoVal(1),KRhoVal(2),KRhoVal(3) )
C   
C
...
C
C
C energy calculations for cell
C
      Ustrain = ZERO
      Umotchem = ZERO
      Umotor = ZERO
...
C
      DO K=1, NDI
      TraceStrainC = TraceStrainC + KStran(K)
      TraceRho = TraceRho + KRho(K)
      TraceSigma = TraceSigma + STRESS(K)
      END DO
      DO K=NTENS+1, NTENS+NTENS
      TraceStrainT = TraceStrainT + KStran(K)
      END DO
C
C
C
C
C
C stability criteria in case of deviatoric contractility
C     IF ((TraceRho - 3.0D0*Rho0*(TEMP+DTEMP)) < ZERO) THEN
C     CALL XIT()
C      END IF
C
C
...
C
C strain energy
      Ustrain = HALF * KmodC1 * (TraceStrainC**2.0D0)
      DO K=1, NDI
      Ustrain = Ustrain + GmodC1 * (KStran(K) - TraceStrainC/NDI)**2.0D0
      END DO
      DO K=NDI+1, NTENS
      Ustrain = Ustrain + 2.0D0 * GmodC1 * (KStran(K)/2.0D0)**2.0D0
      END DO
      STATEV(71) = Ustrain
C
C motor chemical energy
      Umotchem = 0.5D0 * BetaV/3.0D0 * (TraceRho - 3.0D0*Rho0*(TEMP+DTEMP))**2.0D0
      STATEV(78) = Umotchem
      DO K=1, NDI
      Umotchem = Umotchem + 0.5D0 * BetaD * (KRho(K) - TraceRho/NDI)**2.0D0
      END DO
      DO K=NDI+1, NTENS
      Umotchem = Umotchem + 2.0D0 * 0.5D0 * BetaD * KRho(K)**2.0D0
      END DO
      STATEV(79) = Umotchem - STATEV(78)
      STATEV(72) = Umotchem
C
C motor work energy
      Umotor = (TraceRho * TraceStrainC) / 3.0D0
      DO K=1, NDI
      Umotor = Umotor + (KRho(K) - TraceRho/NDI) * (KStran(K) - TraceStrainC/NDI)
      END DO
      DO K=NDI+1, NTENS
      Umotor = Umotor + 2.0D0 * KRho(K) * (KStran(K)/2.0D0)
      END DO
      STATEV(73) = Umotor
C
C mechanical feedback energy
      Umechfeed = (-AlphaV/3.0D0) * (1.0D0/2.0D0) * TraceSigma * TraceRho
      DO K=1, NDI
      Umechfeed = Umechfeed - AlphaD * (KRho(K) - TraceRho/NDI) * (STRESS(K) - TraceSigma/NDI) / 2.0D0
      END DO
      DO K=NDI+1, NTENS
      Umechfeed = Umechfeed - 2.0D0 * AlphaD * KRho(K) * STRESS(K) / 2.0D0
      END DO
      STATEV(74) = Umechfeed
C
C external work energy
      Uext = - (1.0D0/3.0D0) * (1.0D0/2.0D0) * TraceSigma * TraceStrainC
      DO K=1, NDI
      Uext = Uext - (STRESS(K) - TraceSigma/NDI) * (KStran(K) - TraceStrainC/NDI) / 2.0D0
      END DO
      DO K=NDI+1, NTENS
      Uext = Uext - 2.0D0 * STRESS(K) * (KStran(K)/2.0D0) / 2.0D0
      END DO
      STATEV(75) = Uext
C
C actin energy (linear)
      Uactin = HALF * KmodC2 * (TraceStrainT**2.0D0)
      DO K=NTENS+1, NTENS+NDI
      Uactin = Uactin + GmodC2 * (KStran(K) - TraceStrainC/NDI)**2.0D0
      END DO
      DO K=NTENS+NDI+1, NTENS+NTENS
      Uactin = Uactin + 2.0D0 * GmodC2 * (KStran(K)/2.0D0)**2.0D0
      END DO
C     Uactin = 0
C actin energy (nonlinear)
      DO K=NTENS+1, NTENS+NDI
      CALL WPrinStrainTension(KStran(K) , EmodCF , StranCCell , 
     1                   MMCell , NNCell , RatioCell, UactinF)
      Uactin = Uactin + ABS(UactinF)
      END DO
      STATEV(76) = Uactin
C
C total free energy
      Utotal = Ustrain + Umotchem + Umotor + Umechfeed + Uext + Uactin
      STATEV(77) = Utotal
C
C
C
...
C
C     EmodF = 250.0D0 * EmodLamin * (ONE-EnuLamin) 
C    1                          / ((ONE+EnuLamin)*(ONE-TWO*EnuLamin))
           EmodF = 0.0D0 * EmodLamin * (ONE-EnuLamin) 
...
C
      DO K=1, NDI
      TraceStrainE = TraceStrainE + KVal(K)
      END DO
C
C ECM energy (linear)
      Uecm = HALF * Kmod * (TraceStrainE**2.0D0)
      DO K=1, NDI
      Uecm = Uecm + Gmod * (KVal(K) - TraceStrainE/NDI)**2.0D0
      END DO
      DO K=NDI+1, NTENS
      Uecm = Uecm + 2.0D0 * Gmod * (KVal(K)/2.0D0)**2.0D0
      END DO
C     Uecm = 0
C ECM energy (nonlinear)
      DO K=1, NDI
      CALL WPrinStrainTension(KVal(K) , EmodF , StranC , 
     1                   MM , NN , Ratio, UecmF)
      Uecm = Uecm + ABS(UecmF)
      END DO
C
C total ECM free energy
      STATEV(77) = Uecm
C
C
C
...
C
      SUBROUTINE WPrinStrainTension(X , EF , StranC , MM , NN , 
     1                                    Ratio , Y)
C
      INCLUDE 'ABA_PARAM.INC'
C
...
C     
      IF (X .LT. StranC-HALF*StranT) THEN
          Y = ZERO
      ELSE IF (X .LT. StranC+HALF*StranT) THEN
          Y=EF * (StranT**3) / ((NN + 1) * (NN + 2) * (NN + 3)) *
     1    ((X - (StranC - 0.5 * StranT)) / StranT)**(NN + 3)
      ELSE
          Y=EF * StranT * StranT * X / ((NN + ONE) * (NN + TWO)) +
     1     EF * ((ONE + X - (StranC + HALF * StranT))**(MM + THREE) /
     2     ((MM + ONE) * (MM + TWO) * (MM + THREE)) -
     3     ONE * X / ((MM + ONE) * (MM + TWO))) +
     4     EF * (((StranC + HALF * StranT) * X) - 0.5 * X**2) / (MM + ONE) +
     5     EF * (0.5 * (X - (StranC + HALF * StranT))**2 * StranT) / (NN + ONE)
      END IF
C
      RETURN
      END
